...
const fs = require('fs');
const pluginName = 'AngularWordpressApp';
...
distFilenames = fs.readdirSync(`${destination}/dist/angular-wordpress-app/browser/`);
...
const updateLine = (line, name) => {
    const matchedLinePart = line.match(/(?<=browser\/).*?(?=\')/gs).toString();
